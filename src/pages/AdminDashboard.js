...
const GALLERY_CATEGORIES = ['conversion', 'upgrade', 'electrical', 'bodywork', 'painting', 'interior'];
...
export default function AdminDashboard() {
  ...
  const s = { fontFamily: 'Inter, sans-serif', background: '#111', minHeight: '100vh', color: '#e8e8e8' };
    ...
    <div style={s}>
      ...
      <div style={{ background: '#1a1a1a', borderBottom: '1px solid rgba(59,174,232,0.15)', padding: '0.9rem 1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 100 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <img src="/images/logo.jpeg" alt="NEOS" style={{ height: '32px', borderRadius: '5px' }} />
      ...
      <div style={{ maxWidth: '1300px', margin: '0 auto', padding: '2rem 1.5rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.5rem' }}>
          {[{ key: 'contacts', label: `Enquiries${newContacts > 0 ? ` (${newContacts} new)` : ''}` }, { key: 'gallery', label: 'Gallery' }].map(t => (
            <button key={t.key} onClick={() => setTab(t.key)} style={{
              background: tab === t.key ? '#3BAEE8' : 'transparent',
              color: tab === t.key ? '#fff' : '#888',
              border: `1px solid ${tab === t.key ? '#3BAEE8' : 'rgba(255,255,255,0.1)'}`,
                  ...
                  return (
                    <tr key={c.id} onClick={() => setSelected(selected?.id === c.id ? null : c)} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)', cursor: 'pointer', background: selected?.id === c.id ? 'rgba(59,174,232,0.05)' : 'transparent', transition: 'background 0.2s' }}
                      ...
                      <td style={{ padding: '0.85rem 1rem' }}>
                        <span style={{ background: sc.bg, color: sc.color, fontSize: '0.72rem', fontWeight: 700, padding: '0.25rem 0.65rem', borderRadius: '50px' }}>{sc.label}</span>
                      </td>
        ...
        <div style={{ background: '#1a1a1a', border: '1px solid rgba(59,174,232,0.15)', borderRadius: '14px', padding: '1.5rem', height: 'fit-content', position: 'sticky', top: '80px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginBottom: '1.5rem' }}>
            {[
              ...
              <div key={f.label}>
                ...
                {f.href ? (
                  <a href={f.href} style={{ color: '#3BAEE8', fontWeight: 500, fontSize: '0.875rem' }}>{f.value}</a>
                ) : (
                  <p style={{ color: '#e8e8e8', fontSize: '0.875rem', fontWeight: 500 }}>{f.value}</p>
                )}
              </div>
            ))}
            {selected.message && (
            ...
            <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap' }}>
              {Object.entries(STATUS_COLORS).map(([key, sc]) => (
                <button key={key} onClick={() => updateStatus(selected.id, key)} style={{
                  background: selected.status === key ? sc.bg : 'transparent',
                  color: selected.status === key ? sc.color : '#555',
                  border: `1px solid ${selected.status === key ? sc.color : 'rgba(255,255,255,0.08)'}`,
                  borderRadius: '6px', padding: '0.3rem 0.7rem', cursor: 'pointer',
                  fontSize: '0.75rem', fontWeight: 600, fontFamily: 'inherit', transition: 'all 0.2s',
                }}>{sc.label}</button>
              ))}
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
            <a href={`https://wa.me/${selected.phone?.replace(/\D/g, '')}`} target="_blank" rel="noopener noreferrer"
              style={{ flex: 1, background: '#25D366', color: '#fff', border: 'none', borderRadius: '8px', padding: '0.6rem', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600, fontSize: '0.8rem', textAlign: 'center', display: 'block' }}>
              WhatsApp
            </a>
            <a href={`tel:${selected.phone}`} style={{ flex: 1, background: '#3BAEE8', color: '#fff', border: 'none', borderRadius: '8px', padding: '0.6rem', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600, fontSize: '0.8rem', textAlign: 'center', display: 'block' }}>
              Call
            </a>
            <button onClick={() => deleteContact(selected.id)} style={{ background: 'rgba(232,90,90,0.1)', color: '#e85a5a', border: '1px solid rgba(232,90,90,0.2)', borderRadius: '8px', padding: '0.6rem 0.9rem', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600, fontSize: '0.8rem' }}>
              Delete
            </button>
          </div>
        </div>
      ...
      {showAdd && (
        <div style={{ background: '#1a1a1a', border: '1px solid rgba(59,174,232,0.2)', borderRadius: '14px', padding: '1.5rem', marginBottom: '1.5rem', animation: 'fadeInUp 0.3s ease' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem' }}>
            {[
              { key: 'title', label: 'Title *', placeholder: 'e.g. Range Rover Engine Swap' },
              { key: 'car_brand', label: 'Car Brand', placeholder: 'e.g. Range Rover' },
          ...
          <div style={{ marginTop: '1rem' }}>
            ...
            <input type="file" accept="image/*" onChange={async (e) => {
              ...
              if (file) { const url = await uploadImage(file); if (url) setNewItem(n => ({ ...n, image_url: url })); }
            }} style={{ color: '#e8e8e8', fontSize: '0.875rem' }} />
          ...
          {gallery.map(item => (
            <div key={item.id} style={{ background: '#1a1a1a', border: '1px solid rgba(59,174,232,0.1)', borderRadius: '12px', overflow: 'hidden' }}>
              ...
              <div style={{ padding: '1rem' }}>
                ...
                <button onClick={() => deleteItem(item.id)} style={{ marginTop: '0.75rem', background: 'rgba(232,90,90,0.08)', color: '#e85a5a', border: '1px solid rgba(232,90,90,0.15)', borderRadius: '6px', padding: '0.35rem 0.75rem', cursor: 'pointer', fontFamily: 'inherit', fontSize: '0.75rem', fontWeight: 600 }}>
                  Remove
                </button>
              </div>
            </div>
