...
const BRANDS = ['Toyota', 'Lexus', 'Mercedes-Benz', 'Range Rover', 'Other'];
const CONTACT_METHODS = ['WhatsApp', 'Phone Call', 'Email'];

const inputStyle = {
  width: '100%', background: 'var(--bg-card)', border: '1px solid var(--border)',
  borderRadius: '10px', padding: '0.85rem 1rem', color: 'var(--text-primary)',
...
export default function Contact() {
  const [form, setForm] = useState({
    name: '', email: '', phone: '', car_brand: '', car_model: '',
    service_type: '', preferred_contact: '', message: '',
  });
      ...
      <section style={{ background: 'linear-gradient(135deg, #0d1a26 0%, var(--bg-primary) 100%)', padding: '5rem 0 4rem' }}>
        <div className="container">
          ...
          <p className="section-subtitle">
            Tell us what you're driving and what you want done — we'll come back to you with a clear plan and honest quote.
          </p>
        </div>
      </section>
      ...
      <section className="section-padding">
        <div className="container">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '4rem' }}>
            ...
            {/* Contact Info */}
            <div>
              <h2 style={{ fontWeight: 700, fontSize: '1.3rem', marginBottom: '2rem', color: 'var(--text-primary)' }}>Contact Information</h2>

              <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', marginBottom: '3rem' }}>
                <ContactItem icon="📞" label="Phone" value="+234 816 246 5247" href="tel:+2348162465247" />
              ...
              <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '16px', padding: '2rem' }}>
                ...
                {['Toyota', 'Lexus', 'Mercedes-Benz', 'Range Rover'].map(b => (
            ...
            <div>
              ...
              {status === 'success' ? (
                <div style={{ background: 'rgba(59,174,232,0.08)', border: '1px solid var(--accent-blue)', borderRadius: '16px', padding: '3rem', textAlign: 'center', animation: 'fadeInUp 0.4s ease' }}>
                  ...
                  <h3 style={{ fontWeight: 700, fontSize: '1.2rem', marginBottom: '0.5rem', color: 'var(--text-primary)' }}>Message Received!</h3>
                  <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', lineHeight: 1.7 }}>
                    We've received your enquiry and will get back to you within 24 hours. For faster response, WhatsApp us directly.
                  </p>
                  <a href="https://wa.me/2348162465247" target="_blank" rel="noopener noreferrer" className="btn-primary" style={{ marginTop: '1.5rem', display: 'inline-flex' }}>
                ...
                <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                    ...
                    <div>
                      ...
                      <input name="car_model" value={form.car_model} onChange={handle} onFocus={handleFocus} onBlur={handleBlur} required placeholder="e.g. Land Cruiser 200" style={inputStyle} />
                    </div>
                  </div>
                    ...
                    <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
                      {CONTACT_METHODS.map(m => (
                        <label key={m} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', cursor: 'pointer', color: form.preferred_contact === m ? 'var(--accent-blue)' : 'var(--text-secondary)', fontSize: '0.875rem', fontWeight: form.preferred_contact === m ? 600 : 400 }}>
                          <input type="radio" name="preferred_contact" value={m} checked={form.preferred_contact === m} onChange={handle} style={{ accentColor: 'var(--accent-blue)' }} required />
                          {m}
                        </label>
                      ))}
                    </div>
                  ...
                  {status === 'error' && (
                    <p style={{ color: '#e85a5a', fontSize: '0.85rem', background: 'rgba(232,90,90,0.08)', padding: '0.75rem 1rem', borderRadius: '8px', border: '1px solid rgba(232,90,90,0.2)' }}>
